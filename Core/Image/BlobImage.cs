...
namespace Core.Image
{
    public class BlobImage : IBlobImage
    {
        ...
        public BlobImage(BlobImageEntity blobImageEntity)
        {
            Image = Mat.FromArray(blobImageEntity.Image);
            Indexer = Image.GetGenericIndexer<int>();
            tags = blobImageEntity.Tags;
        }
        ...
        public void GenerateBGRAImage(Mat colorImage, IBlobId2ColorConverterService converter)
        {
        ...
        /// <returns></returns>
        public Mat GenerateBGRAImage(IBlobId2ColorConverterService converter)
        {
        ...
        public BlobImageProxy GetProxy()
            => new BlobImageProxy(this);

        #endregion

        public BlobImageEntity ToBlobImageEntity()
        {
            Image.GetRectangularArray(out int[,] image);
            return new BlobImageEntity
            {
                Image = image,
                Tags = Tags
            };
